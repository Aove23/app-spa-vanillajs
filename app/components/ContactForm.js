  ...
  function validationsForm() {
    ...
    d.addEventListener("keyup", (e) => {
      if (e.target.matches(".contact-form [required]")) {
        //Es el input que origino el evento para no repetir tantas veces e.target
        let $input = e.target,
          //Valida el patron que viene del input o del textarea(porque los textarea no tienen atributos patron)
          pattern = $input.pattern || $input.dataset.pattern;

        //Validamos los inputs que tienen patron y que si estan vacio que todavia no validen
        ...
        //Validamos los inputs o textarea que no tienen patron
        if (!pattern) {
          return $input.value === ""
            ? d.getElementById($input.name).classList.add("is-active")
            : d.getElementById($input.name).classList.remove("is-active");
